...
<script lang='ts'>
import { IEventTarget, TImage, IVSelectItem, INewVendor } from "@/types";
...
export default class BusinessForm extends Vue {
  mounted() {
    if (this.edit && this.vendor.id) {
      this.form.id = this.vendor.id;
      this.form.title = this.vendor.title;
      this.form.image = this.vendor.image?.paths.sm as string;
      if (this.form.image) {
        // TODO: Remove TS ignore
        // eslint-disable-next-line @typescript-eslint/ban-ts-ignore
        // @ts-ignore1
        this.imgPreview = this.form.image;
